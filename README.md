...
学生チーム WASSYOI のポートフォリオサイト。営業資料 QR の遷移先として、"お祭り"主催者・実行委員会・自治体担当者向けに事業内容（Mission / Vision / Value）・実績・問い合わせ導線をまとめた、GitHub Pages 配信前提の静的サイトです。
...
- 事例の追加: `index.html` の `.wp-cases` 内に `<article class="wp-case">`（または `wp-case wp-case--reverse`）をもう 1 件複製し、画像・日付・地名・タイトル・本文・タグ・サイト URL を差し替える
...
このサイトは営業資料 QR からの遷移先。エンジニア向けの情報（GitHub・技術スタック・パス・バージョン番号）は出さず、"お祭り"主催者・実行委員会・自治体担当者が「WASSYOI に頼むと何ができるか」をすぐに掴める表現に揃える。

モダンなビジネスサイト寄りのトーンで、祭装飾（ハチマキ・ハーフトーン）はアクセント程度に絞る。
